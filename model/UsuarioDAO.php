...
class UsuarioDAO implements IDAO {
	...
	private $cli;
	private $tabela;
	function __construct(Usuario $cli) {
	...
	function logar() {
		$this->cli->__set('senha',md5($this->cli->__get('senha').'1yx8k45gxc'));
		$qry = $this->dao->prepare ( "select * from $this->tabela where login=:login and senha=:senha" );
		$qry->execute ( $this->cli->toArray () );
		$dados = $qry->fetch ( PDO::FETCH_ASSOC );
		return $dados;
	}
